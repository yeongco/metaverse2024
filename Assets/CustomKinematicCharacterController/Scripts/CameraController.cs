...
public class CameraController : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (cameraMode == ECameraMode.ThirdPerson) {
            ...
            if (kcc.Velocity.magnitude > 0.1 && kcc.IsGrounded)
            {
                float yOffset = Inity + Mathf.Sin(Time.time * UpDownSpeed) * Inity; // 시간에 따라 위아래로 흔들리도록 함
